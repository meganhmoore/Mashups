	...
	if (spaceData.ready){
		...
		if (spaceData.apiData.number === 0){
			...
			for (var i=0; i<spaceData.apiData.number; i++) {
				var curPersonObj = spaceData.apiData.people[i];
				astros[i] = new Astronaut(random(width), random(height), random(20,70), random(70,120), random(-3,3), random(-3,3), curPersonObj);
			}
			//Toggle the booleans
			spaceData.ready = false;
...
//Astronaut Class
function Astronaut(xPos, yPos, xWidth, yHeight, xSpeed, ySpeed, personObj){
...
Astronaut.prototype.clicked = function(){
	...
	$('#spaceName').html(this.person.name);
};
