...
function makeD3Chart(dataset){
	...
	var h = 300;
	var barPadding = 2;//controls the space between the rectangles
	...
	var yScale = d3.scaleLinear()//scaling linearly to put them into a new range
		.domain([dataMin, dataMax])
		.range([50,h - 50]);//lowest one goes to 50, highest one goes to the top-50
	...
	svg.selectAll("rect")
		...
		.enter()
		.append("rect")
		.attr("x", function(d, i) {//d=object(temps) i=index(day in this case)
		...
		.attr("fill", function(d){
		 ...
		 .on('click', function(d){
			...
			//data binding!!!
			//$('h1').html("The temp is "+ d.day);
			$('h1').html("The temp at night will be "+ d.eve);

		//ANIMATE THE BARS ON THE GRAPH
		// 	d3.select(this)
		// 		.transition()
		// 		.attr("y", h)
		// 		.attr("fill", "blue")
...
$(document).ready(function(){
	...
	//Redraw on resize, reset bars and their values
